...
public interface FriendRepository extends JpaRepository<Friend, Long> {
    ...
    Optional<Friend> findFriendRequestBySenderIdAndReceiverIdAndStatus(
    ...
    /**
     * memberId에 해당하는 회원과 승인된 친구 관계인 회원들을 찾는 메서드입니다. 승인된 friend 중에서 memberId에 해당하는 회원이 sender인 경우와
     * receiver인 경우를 모두 찾고, 그 상대방 회원을 반환합니다.
     *
     ...
     * */
    @Query("SELECT f FROM Friend f " +
            "WHERE (f.receiver.id = :receiverId AND f.sender.id = :senderId AND f.friendRequestStatus != 'REJECTED') " +
            "OR (f.sender.id = :receiverId AND f.receiver.id = :senderId AND f.friendRequestStatus != 'REJECTED')")
