...
/**
 * 알림 관련 요청을 처리하는 컨트롤러입니다. 이 클래스는 알림 리스트 조회, 안 읽은 알림 상태 확인, 특정 알림 읽음 처리 등의 API를 제공합니다.
 */
@AllArgsConstructor
@RestController
...
public class NotificationController {
    ...
    /**
     * 사용자의 안 읽은 알림이 있는지 확인하는 API입니다. HTTP 상태 코드 200(OK)와 함께 안 읽은 알림 여부를 반환합니다.
     *
    ...
    /**
     * 특정 알림을 읽음으로 처리하는 API입니다. 해당 알림 ID를 받아 알림을 읽음 상태로 변경하고, 변경된 알림 정보를 반환합니다.
     *
     * @param notificationId 읽을 알림의 ID
     * @return ApiResponse<Void> 형태로 읽음 처리된 알림 정보를 반환
     */
    @PatchMapping("/{notificationId}/read")
    @ResponseStatus(HttpStatus.OK)
    public ApiResponse<Void> notificationRead(
